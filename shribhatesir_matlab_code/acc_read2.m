...
hFig = figure('Toolbar','none','Menubar', 'none',...
    ...
    'Position',[200 250 350 150],'Color',[0.6 0.6 0.6]);
axis off; 
...
%%  connect_callback
    ...
    if (length(cell2mat(getAvailableComPort()))> 2)
    exitflag=0;    
    fclose(instrfindall);
    delete(instrfindall);
...
%         set(zmax,'string',max(alls3));
        ...
        set(xdeg,'string',round(dd1));
        set(ydeg,'string',round(dd2));
        set(zdeg,'string',round(dd3));
